-- Fecha um buraco real: um estafeta recem-criado que nunca chegou a abrir a
-- app (ultima_atividade_em = null) passava no filtro de "esteve ativo
-- recentemente" (a condicao aceitava null OU recente), por isso podia
-- receber pedidos automaticamente mesmo sem nunca ter existido de verdade
-- do lado do telemovel. Agora exige atividade real e recente -- null deixa
-- de contar como "ok".
      ...
      where o.loja_id = v_loja.idloja
        ...
        and (
          ...
          or (
            ...
            and o.scheduled_for > now() - interval '24 hours'
          )
        )
    ...
    loop
      ...
      select e.id, public.estafeta_haversine_km(v_loja.latitude, v_loja.longitude, e.ultima_localizacao_lat, e.ultima_localizacao_lng)
        ...
        where e.ativo = true and e.eliminado = false and e.online = true and e.disponivel = true
          and e.ultima_atividade_em is not null and e.ultima_atividade_em > now() - interval '90 seconds'
        order by
