-- Prova de entrega: bucket de storage novo (mesmo padrao de store-images/
-- menu-images -- publico para leitura, escrita so via signed URL emitida
-- pela edge function request-image-upload, nunca direto do browser) e coluna
-- nova em atribuicoes_entrega para guardar o URL da foto.

insert into storage.buckets (id, name, public)
values ('delivery-proofs', 'delivery-proofs', true)
on conflict (id) do nothing;

drop policy if exists "Ler provas de entrega" on storage.objects;
create policy "Ler provas de entrega"
  on storage.objects for select
  to public
  using (bucket_id = 'delivery-proofs');

alter table public.atribuicoes_entrega
  add column if not exists prova_entrega_foto_url text;

-- estafeta_advance_status ganha foto_url_input: obrigatorio para o proprio
-- estafeta ao marcar "entregue" (nao para admin, que tem admin_force_deliver_assignment
-- para os casos em que a foto nao e possivel).
-- Nota: adicionar um parametro muda a assinatura -- "create or replace" criaria
-- uma segunda funcao sobrecarregada em vez de substituir a antiga, deixando a
-- versao de 3 argumentos (sem exigencia de foto) chamavel na mesma. Remover
-- primeiro.
drop function if exists public.estafeta_advance_status(integer, bigint, text);
