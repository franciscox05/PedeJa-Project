-- Fase 1.8 (reestruturacao base44): tarefas simples de recrutamento/equipa
-- (seguimento de candidaturas de restaurantes, contactos, etc), so para uso
-- interno do admin. Mesmo padrao SECURITY DEFINER + is_platform_admin usado
-- em 055_store_category_assignment.sql.
...
create index if not exists idx_tarefas_recrutamento_status on public.tarefas_recrutamento (status);
...
revoke all on function public.admin_list_recruitment_tasks(integer) from public;
revoke all on function public.admin_upsert_recruitment_task(integer, bigint, text, text, text, text, text, text, date, text, text) from public;
revoke all on function public.admin_delete_recruitment_task(integer, bigint) from public;
grant execute on function public.admin_list_recruitment_tasks(integer) to anon, authenticated, postgres, service_role;
grant execute on function public.admin_upsert_recruitment_task(integer, bigint, text, text, text, text, text, text, date, text, text) to anon, authenticated, postgres, service_role;
grant execute on function public.admin_delete_recruitment_task(integer, bigint) to anon, authenticated, postgres, service_role;
